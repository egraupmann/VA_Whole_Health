...
4) acu_outpatient_diagnoses
5) acu_patient_info
6) acu_inpatient_diagnoses
7) acu_vital_signs
8) mh_factors
***************************************************************/
...
select distinct
	...
	,dssl.PrimaryStopCode as DSSLocationPrimaryStopCode
	,dssl.SecondaryStopCode as DSSLocationSecondaryStopCode
	,dst.NationalChar4
	...
	,case when inst.LocationName LIKE '%battlefield%' or inst.LocationName LIKE '%bfa%' 
		  ...
		  when inst.LocationName like '%acup%' or inst.LocationName like '%acpu%' then 'trad'
		  else 'unknown'
		  end as acu_loc_type
...
select distinct
	...
	,v.VisitSID
	,p.PatientICN
	,hf.VisitDateTime, convert(date, hf.VisitDateTime) as VisitDate
	,v.InstitutionSID
	,v.LocationSID
	...
	,ssc.StopCode as SecondaryStopCode
	,ssc.StopCodeName as SecondaryStopCodeName
	,case when hft.HealthFactorType like '%bfa%' or hft.HealthFactorType like '%battlefield%' then 'bfa'
		  when hft.HealthFactorType like '%acup%' or hft.HealthFactorType like '%acpu%' then 'trad'
		  else 'unknown'
		  end as hf_acu_type
into #acu_outpatient_visits
from CDWWork.Outpat.Visit as v
	inner join CDWWork.Patient.Patient as p on v.PatientSID = p.PatientSID
	...
	inner join CDWWork.SPatient.SPatient as sp on v.PatientSID = sp.PatientSID
where 
	(hft.HealthFactorType like '%acup%' or hft.HealthFactorType like '%acpu%' or  hft.HealthFactorType like '%bfa%' or hft.HealthFactorType like '%battlefield%')
	...
	and sp.CDWPossibleTestPatientFlag != 'Y' and sp.VeteranFlag = 'Y'
	and (hf.VisitDateTime between convert(datetime2(0),'2022-10-01 00:00:00.000') and convert(datetime2(0),'2025-09-30 23:59:59.999'))
...
from CDWWork.Outpat.VProcedure as op
	inner join CDWWork.Patient.Patient as p on p.PatientSID = op.PatientSID
	inner join CDWWork.Dim.CPT as cpt on op.CPTSID = cpt.CPTSID
	inner join CDWWork.SPatient.SPatient as sp on op.PatientSID = sp.PatientSID
where (op.VisitDateTime between convert(datetime2(0),'2022-10-01 00:00:00.000') and convert(datetime2(0),'2025-09-30 23:59:59.999')) 
...
select distinct 
	...
	,case when dd.TIUDocumentDefinition like '%battlefield%' or dd.TIUDocumentDefinition like '%bfa%' then 'bfa'
		  when dd.TIUDocumentDefinition like '%acup%' or dd.TIUDocumentDefinition like '%acpu%' then 'trad'
		  end as NoteType
into #acu_doc_name
from cdwwork.TIU.TIUDocument as d
	inner join CDWWork.Patient.Patient as p on p.PatientSID = d.PatientSID
	inner join cdwwork.dim.TIUDocumentDefinition as dd on d.TIUDocumentDefinitionSID = dd.TIUDocumentDefinitionSID
where
	((dd.TIUDocumentDefinition like '%battlefield%' or dd.TIUDocumentDefinition like '%bfa%')
			and dd.TIUDocumentDefinition not like '%note%'
		or ((dd.TIUDocumentDefinition like '%acup%' or dd.TIUDocumentDefinition like '%acpu%')
			and dd.TIUDocumentDefinition not like '%battlefield%' and dd.TIUDocumentDefinition not like '%BFA%'))
...
where (a.VisitDateTime between convert(datetime2(0),'2022-10-01 00:00:00.000') and convert(datetime2(0),'2025-09-30 23:59:59.999'))
...
***************************************************************/
with visit_list as (
	select distinct PatientSid, PatientICN, VisitSID, VisitDate from #acu_docs
	union
	select distinct PatientSid, PatientICN,, VisitSID, VisitDate from #acu_outpatient_procedures
	union
	select distinct PatientSid, PatientICN,, VisitSID, VisitDate from #acu_outpatient_visits
	union
	select distinct PatientSid, PatientICN,, VisitSID, VisitDate  from #name_only
	union
	select distinct PatientSid, PatientICN,, VisitSID, VisitDate  from #CHAR4
	)
...
select distinct A.PatientSid
	...
	, case when c.NoteType = 'bfa' or e.acu_loc_type = 'bfa' or f.acu_btf_CHAR4 = 1 or d.hf_acu_type='bfa' Then 1 Else 0 end as 'BFA'
into #acu_visits
from visit_list A
...
/***************************************************************
Get the various additional demographic and detail extracts, all
filtered using the patients who had acupuncture visits, based
on the compendium logic
***************************************************************/
...
select distinct
	...
	,pg.PriorityGroupCode
	,pg.PriorityGroupName
	,pg.PriorityGroupDescription
	,sg.PrioritySubGroupCode
	,sg.PrioritySubGroupName
	,sg.PrioritySubGroupDescription
	,en.EffectiveDate
	,en.ServiceConnectedPercentage
	,en.EnrollmentDate
	,en.EnrollmentEndDate
...
from CDWWork.Patient.Patient as p
	...
	inner join CDWWork.Veteran.ADRPerson as vet on vet.ADRPersonICN = p.PatientICN
	left join CDWWork.ADR.ADREnrollHistory as eh on vet.ADRPersonSID = eh.ADRPersonSID
	left join CDWWork.ADR.ADRPriorityGroup as pg on eh.ADRPriorityGroupID = pg.ADRPriorityGroupID
	left join CDWWork.ADR.ADRPrioritySubGroup as sg on eh.ADRPrioritySubGroupID = sg.ADRPrioritySubGroupID
	left join CDWWORK.ADR.enrollment as en on p.PatientSID = en.PatientSID
	inner join #acu_patient_list as acu on p.PatientICN = acu.PatientICN
