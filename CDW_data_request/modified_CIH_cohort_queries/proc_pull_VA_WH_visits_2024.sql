...
drop proc if exists WH_CIH.pull_VA_WH_visits;
go
create proc WH_CIH.pull_VA_WH_visits (
...
FROM #consolidated_methods M
	INNER JOIN [Src].[Outpat_VProcedure] A
...
into #HF_Excl
from #consolidated_methods M
	INNER JOIN [Src].[HF_HealthFactor] A
		on M.VisitSID = A.VisitSID
	INNER JOIN ORD_Fix_202309007D.WH_CIH.HealthFactors L 
...
SELECT ScrSSN
	...
	, CHAR4_code
	, StopCode
INTO WH_CIH.temp_consolidated_methods
FROM #consolidated_methods_excl2
