...
/* last updated: 8/4/26 */
/* the original code for this was in the get_acupuncture_visits.sql file, but it was moved here to make it easier to update the 
	Dim tables without having to run the entire get_acupuncture_visits.sql file. This code was provided by Jamie Douglas at VA*/
/*
Dim tables:
	- CPT
	- Stop codes
...
DROP TABLE IF EXISTS OPCCCT_CIH.dflt.CPT;
SELECT CPTSID
	, Sta3n
	, CPTCode
	, CPTName
	...
	, CASE 
		...
		END AS CIHType
	, StartDate = CONVERT(date, '2022-10-01')
	, EndDate   = CONVERT(date, '2050-01-01')
INTO OPCCCT_CIH.dflt.CPT
...
drop table if exists OPCCCT_CIH.dflt.StopCodes;
select distinct StopCodeSID
	...
	, StartDate = CONVERT(date, '2022-10-01')
	, EndDate   = CONVERT(date, '2050-01-01')
into OPCCCT_CIH.dflt.StopCodes
...
DROP TABLE IF EXISTS OPCCCT_CIH.dflt.HealthFactors;
with intermed as (
	SELECT * FROM #HFAcupTrad
	union
...
select * INTO OPCCCT_CIH.dflt.HealthFactors FROM INTERMED;
...
DROP TABLE IF EXISTS OPCCCT_CIH.dflt.LocationNames;
with intermed as (
	SELECT * FROM #LNAcupTrad
	union
	SELECT * FROM #LNAcupBFA
...
select * INTO OPCCCT_CIH.dflt.LocationNames FROM INTERMED;
...
DROP TABLE IF EXISTS OPCCCT_CIH.dflt.NoteTitles;
with intermed as (
	SELECT * FROM #NTAcupTrad
	union
	SELECT * FROM #NTAcupBFA
...
select * INTO OPCCCT_CIH.dflt.NoteTitles FROM INTERMED;
...
DROP TABLE IF EXISTS OPCCCT_CIH.dflt.CIHWHLocationOnlyExclusions;
with intermed as (
...
select * INTO OPCCCT_CIH.dflt.CIHWHLocationOnlyExclusions FROM INTERMED;
...
DROP TABLE IF EXISTS OPCCCT_CIH.dflt.CIHWHNoteTitleOnlyExclusions;
with intermed as (
	SELECT * FROM #NT_only_Acup
	union 
	SELECT * FROM #NT_only_Biofeedback
...
select * INTO OPCCCT_CIH.dflt.CIHWHNoteTitleOnlyExclusions FROM INTERMED;
...
drop table if exists OPCCCT_CIH.dflt.Telehealth
...
into OPCCCT_CIH.dflt.Telehealth
